...
Observeu que per comprovar si els dos nombres són consecutius utilitzem `(x + 1) % 100000 == y or (y + 1) % 100000 == x`. Durant el concurs, diversos participants intentaven comprovar aquesta condició amb fórmules com `(x + 1) % 100000 == y or (x - 1) % 100000 == y`. El problema d'això és que quan prenem el mòdul d'un nombre negatiu, C++ retorna un nombre negatiu. Així doncs, si $x = 0$ i $y = 99999$, la condició `(x - 1) % 100000 == y` s'avaluarà com a `false`, ja que $(0 - 1) % 100000 = -1 \neq 99999$.
